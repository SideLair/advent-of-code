...
def check_bag(bag):
    ...
    for i in range(1, len(colors) - 1):
        inner_bags[colors[i][3:]] = colors[i][1]

    return Bag(colors[0], inner_bags)


def count_gold_bags(bags):
    good_bags = [Bag('shiny gold', {})]

    for good_bag in good_bags:
        for bag in bags:
            if good_bag.color in bag.inner_bags and bag not in good_bags:
                good_bags.append(bag)

    return len(good_bags) - 1 #starting gold shiny one
...
print('Gold bag count:', count_gold_bags(bags))